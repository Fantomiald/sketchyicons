 ...
 * hand written ones.
 *
 * createElement rather than JSX, so the package needs no jsx runtime in either
 * module format and its only peer dependency is react itself.
 */
const createSketchyIcon = (name: string, nodes: IconNode): SketchyIcon => {
  const Icon = forwardRef<SVGSVGElement, SketchyIconProps>(
    (
      {
        color = 'currentColor',
        size = 24,
